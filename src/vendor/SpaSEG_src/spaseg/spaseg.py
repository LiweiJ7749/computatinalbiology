...
from spaseg.spaseg_model import SegNet
from spaseg._utils import get_3d_expMatrix, get_max_H_W, seed_torch, merge_outlier, add_embedding, cal_metric, batch_umap_plot
...
class SpaSEG():
    ...
    def _train(self,
            ...
            for batch_idx in iterator:
                ...
                if n_labels <= self.min_label:
                    logg.info("nLabels {} reached minLabels {}.".format(n_labels,self.min_label))
                    iterator.close() 
                    break
    ...
    def _cal_metrics(self, ground_truth_index):
        self.adata = [cal_metric(adata, ground_truth_index) for adata in self.adata]

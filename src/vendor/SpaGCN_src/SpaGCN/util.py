...
def rank_genes_groups(input_adata, target_cluster,nbr_list, label_col, adj_nbr=True, log=False):
    ...
    # 未校正 p 值（供跨语言统一 pval 列与 p 值直方图评价；原版仅输出 pvals_adj）
    ...
    df = {'genes': genes, 'in_group_fraction': fraction_obs.loc[1].tolist(), "out_group_fraction":fraction_obs.loc[0].tolist(),"in_out_group_ratio":(fraction_obs.loc[1]/fraction_obs.loc[0]).tolist(),"in_group_mean_exp": mean_obs.loc[1].tolist(), "out_group_mean_exp": mean_obs.loc[0].tolist(),"fold_change":fold_change.tolist(), "pvals":pvals, "pvals_adj":pvals_adj}
    df = pd.DataFrame(data=df)
    return df

# coding: utf-8
# reference: https://github.com/tctianchi/pyvenn/blob/master/venn.py
...
def draw_ellipse(fig, ax, x, y, w, h, a, fillcolor,ec='auto', lw=None):
    e = patches.Ellipse(
        xy=(x, y),
        width=w,
        height=h,
        angle=a,
        color=fillcolor, ec=ec, lw=lw)
    ax.add_patch(e)
...
def venn2(labels, names=['A', 'B'], **options):
    ...
    ec = options.get('ec', 'auto')
    lw = options.get('lw', None)
    ...
    # body
    draw_ellipse(fig, ax, 0.375, 0.3, 0.5, 0.5, 0.0, colors[0],ec=ec, lw=lw)
    draw_ellipse(fig, ax, 0.625, 0.3, 0.5, 0.5, 0.0, colors[1],ec=ec, lw=lw)
...
def venn3(labels, names=['A', 'B', 'C'], **options):
    ...
    # body
    draw_ellipse(fig, ax, 0.333, 0.633, 0.5, 0.5, 0.0, colors[0], ec=ec, lw=lw)
    draw_ellipse(fig, ax, 0.666, 0.633, 0.5, 0.5, 0.0, colors[1], ec=ec, lw=lw)
    draw_ellipse(fig, ax, 0.500, 0.310, 0.5, 0.5, 0.0, colors[2], ec=ec, lw=lw)
...
def venn4(labels, names=['A', 'B', 'C', 'D'], **options):
    ...
    # body
     # x, y, w, h, a,
    draw_ellipse(fig, ax, 0.350, 0.400, 0.72, 0.45, 140.0, colors[0], ec=ec, lw=lw)
    draw_ellipse(fig, ax, 0.450, 0.500, 0.72, 0.45, 140.0, colors[1], ec=ec, lw=lw)
    draw_ellipse(fig, ax, 0.544, 0.500, 0.72, 0.45, 40.0, colors[2], ec=ec, lw=lw)
    draw_ellipse(fig, ax, 0.644, 0.400, 0.72, 0.45, 40.0, colors[3], ec=ec, lw=lw)

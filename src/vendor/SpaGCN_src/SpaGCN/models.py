...
class simple_GC_DEC(nn.Module):
    def __init__(self, nfeat, nhid, alpha=0.2, device="cpu"):
        ...
        self.alpha=alpha
        # 训练/推理设备（cuda/cpu），默认 cpu 以保持向后兼容
    ...
    def fit(self, X,adj,  lr=0.001, max_epochs=5000, update_interval=3, trajectory_interval=50,weight_decay=5e-4,opt="sgd",init="louvain",n_neighbors=10,res=0.4,n_clusters=10,init_spa=True,tol=1e-3):
        self.trajectory=[]
        # 设备解析：与构造/SpaGCN.train 传入 device 一致；把已有参数(gc weight/bias)搬到设备
        ...
        # 保留 numpy 副本给 sklearn/louvain 初始化；训练张量按需搬到设备
        X_np = np.asarray(X, dtype=np.float32)
        adj_np = np.asarray(adj, dtype=np.float32)
        features = self.gc(torch.as_tensor(X_np, device=dev), torch.as_tensor(adj_np, device=dev))
        #----------------------------------------------------------------        
        if init=="kmeans":
            ...
            if init_spa:
                ...
                y_pred = kmeans.fit_predict(X_np)  #Here we use X as numpy
        elif init=="louvain":
        ...
        features_df=pd.DataFrame(features_np,index=np.arange(0,features_np.shape[0]))
        Group=pd.Series(y_pred,index=np.arange(0,features_np.shape[0]),name="Group")
        Mergefeature=pd.concat([features_df,Group],axis=1)
        cluster_centers=np.asarray(Mergefeature.groupby("Group").mean())
        # mu 直接建在目标设备（赋值为 Parameter 会被 nn.Module 自动注册）
        self.mu = Parameter(torch.as_tensor(cluster_centers, dtype=torch.float32, device=dev))
        X = torch.as_tensor(X_np, device=dev)
        adj = torch.as_tensor(adj_np, device=dev)
        self.train()
    ...
    def fit_with_init(self, X,adj, init_y, lr=0.001, max_epochs=5000, update_interval=1, weight_decay=5e-4,opt="sgd"):
        ...
        if opt=="sgd":
            ...
            optimizer = optim.Adam(self.parameters(),lr=lr, weight_decay=weight_decay)
        X=torch.as_tensor(np.asarray(X, dtype=np.float32), device=dev)
        adj=torch.as_tensor(np.asarray(adj, dtype=np.float32), device=dev)
        features, _ = self.forward(X,adj)
        features_np = features.detach().cpu().numpy()
        features=pd.DataFrame(features_np,index=np.arange(0,features_np.shape[0]))
        Group=pd.Series(init_y,index=np.arange(0,features_np.shape[0]),name="Group")
        Mergefeature=pd.concat([features,Group],axis=1)
        cluster_centers=np.asarray(Mergefeature.groupby("Group").mean())
        self.mu.data.copy_(torch.as_tensor(cluster_centers, dtype=torch.float32, device=dev))
        self.train()
        for epoch in range(max_epochs):
            if epoch%update_interval == 0:
                _, q = self.forward(X, adj)
    ...
    def predict(self, X, adj):
        # 返回 CPU tensor，兼容下游 torch.argmax(...).data.cpu().numpy() / q.detach().numpy()
        dev = torch.device(self.device)
        z,q = self(torch.as_tensor(np.asarray(X, dtype=np.float32), device=dev),
                   torch.as_tensor(np.asarray(adj, dtype=np.float32), device=dev))
        return z.cpu(), q.cpu()

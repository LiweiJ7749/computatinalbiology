...
class SpaGCN(object):
    ...
    def train(self,adata,adj, 
        ...
        self.res=res
        self.tol=tol
        # 解析训练设备：auto -> 有 CUDA 就用 GPU，否则 CPU
        if device == "auto":
            device = "cuda" if torch.cuda.is_available() else "cpu"
        self.device = device
        print("SpaGCN train on device:", device)
        ...
        #----------Train model----------
        self.model=simple_GC_DEC(embed.shape[1],embed.shape[1], device=self.device)
...
class multiSpaGCN(object):
    ...
    def train(self,adata_list,adj_list, l_list,
        ...
        if device == "auto":
            device = "cuda" if torch.cuda.is_available() else "cpu"
        self.device = device
        print("SpaGCN (multi-section) train on device:", device)
        num_spots=0
        for i in adata_list: 
            num_spots+=i.shape[0]

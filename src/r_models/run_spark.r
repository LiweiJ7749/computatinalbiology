# ============================================================
# SPARK-X 方法：Visium Mouse Olfactory Bulb 数据 SVG 检测
# 输入：results/local_results/Visium_Mouse_Olfactory_Bulb/SPARK_X/ 下的中间文件
#       counts.mtx (genes x spots), genes.csv, barcodes.csv, location.csv
# 输出：SVG_SPARK_Visium_Mouse_Olfactory_Bulb.csv + ggplot2 展示图
# ============================================================
...
data_dir <- if (length(args) >= 1) args[1] else
  "F:/computatinalbiology/results/local_results/Visium_Mouse_Olfactory_Bulb/SPARK_X"
out_dir <- data_dir
...
# ---------- 保存 CSV ----------
csv_path <- file.path(out_dir, "SVG_SPARK_Visium_Mouse_Olfactory_Bulb.csv")
...
p <- ggplot(top_df, aes(x = gene, y = -log10(adjustedPval))) +
  ...
  coord_flip() +
  labs(title = "SPARK-X: Top SVG genes (Visium Mouse Olfactory Bulb)",
       x = "Gene", y = "-log10(adjusted P-value)") +
  theme_minimal(base_size = 11) +
  theme(plot.title = element_text(hjust = 0.5))

png_path <- file.path(out_dir, "SVG_SPARK_Visium_Mouse_Olfactory_Bulb_top.png")
